...
const setDialog = function (component, opts) {
    ...
    dialogComponent.$dialog = Vue.$newDialog(opts);
...
// const ajaxUrl = env === 'production' ? `${location.protocol}//${location.host}/` : 'http://10.0.5.244:9008/';

// const ajaxUrl = process.env.NODE_ENV === 'development' ? 'http://www.mochyli.com/' : '${location.protocol}//${location.host}'
const ajaxUrl = process.env.NODE_ENV === 'development' ? 'http://10.0.5.195:3000/' : `${location.protocol}//${location.host}`
...
// 减去t(天)时间t
util.dateCut = function (date, t) {
    let ds = date - 3600 * 1000 * 24 * t
    // console.log("原来时间====",util.dateToStr(new Date(date)),"减去后的时间====",util.dateToStr(new Date(ds)))
    return util.dateToStr(new Date(ds))
};
...
const systemInfos = {
    ...
    OTC_FROCE_RELEASE_FROM_SYMBOL:"已强制放币",
    OTC_FROCE_RELEASE_TO_SYMBOL:"系统强制放币",
...
// 倒计时 t为剩余秒数
const countTime = function (t, callbackFun) {
    var db = function (m) {
      return m < 10 ? '0' + m : m
    }
    function a() {
...
const outExl = function (url, parmas) {
    ...
    console.log(data,`${util.baseURL}${url}${data.join('&')}`)
    window.location.href = `${util.baseURL}${url}${data.join('&')}`
    // window.location.href = `http://10.0.5.209:9009/${url}${data.join('&')}` //四龙的本地

}
util.outExl = outExl

// 导出表格2
const outExcel = function (url, data, success, error) {
